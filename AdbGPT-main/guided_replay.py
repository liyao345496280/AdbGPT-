...
import cfgs
import config
import view_hierarchy as view_hierarchy
...
class UI():
    ...
    def encoding(self):
        ...
        vh.load_xml(vh_data)
        view_hierarchy_leaf_nodes = vh.get_leaf_nodes() #PS:3
        utils.sortchildrenby_viewhierarchy(view_hierarchy_leaf_nodes, 'bounds') #PS:5
    ...
    def element_encoding(self, _id, _obj_type, _text, _content_desc, _resource_id):
        ...
        if _obj_type in ['CHECKBOX', 'CHECKEDTEXTVIEW', 'SWITCH']:
            ...
            if _class == "":
                _text = _content_desc if _text == "" else _text #PS:6
...
class Guided_Replay():
    ...
    def infer(self, question,Flag):
        if Flag:
            self.chatgpt.infer(question)
        else:
